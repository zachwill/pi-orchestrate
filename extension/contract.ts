...
- **Mandatory wave cardinality:** If the enumerated wave has N workers, your next assistant response must contain exactly N separate, fully briefed \`orchestrate\` invocations. A single invocation is valid only when N=1. Form all N invocations before emitting or finalizing the response: a sole asynchronous invocation ends the parent turn (\`terminate: true\`), so omitted siblings cannot be added afterward.
- **Mandatory response shape:** That wave's assistant response must be pure orchestration: exactly those N \`orchestrate\` invocations and no text or non-orchestration tools. Pi natively supports sibling tool calls. For N=3, emit three native sibling calls in one response: \`orchestrate({ worker, title, instructions })\`, \`orchestrate({ worker, title, instructions })\`, and \`orchestrate({ worker, title, instructions })\`.
