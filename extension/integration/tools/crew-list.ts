...
export function registerCrewListTool({ pi, runtime, notifyDiscoveryWarnings }: CrewToolDeps): void {
  pi.registerTool({
    ...
    promptGuidelines: ["crew_list: Use before crew_spawn to discover worker names and descriptions.", "crew_list: Role boundaries: scout = narrow discovery only; investigator = broad/cross-file synthesis or review; worker = bounded implementation.", "crew_list: Do not poll for completion; worker results arrive as steering messages."],
